...
mergepr <- PR %>% select(hv001,hv002,hvidx,starts_with("sh20g"),
...
SNw <- mutate(SNw,
      ...
      # Disability
      #for dis any
      wnsee = case_when (wsh20gb %in% c(3,4) | wsh20gc %in% c(3,4) ~ 1,
                        (!is.na(wsh20gb) | !is.na(wsh20gc)) & (wsh20gb %in% c(1:4) | wsh20gc %in% c(1:4))  ~ 0),
      #for dis any 2
      wnsee2 = case_when (wsh20gb %in% c(2,3,4) | wsh20gc %in% c(2, 3,4) ~1,
                         (!is.na(wsh20gb) | !is.na(wsh20gc)) & (wsh20gb %in% c(1:4) | wsh20gc %in% c(1:4)) ~ 0),
      
      #for dis any
      wnhear = case_when (wsh20ge %in% c(3,4) | wsh20gf %in% c(3,4) ~ 1, 
                         (!is.na(wsh20ge) | !is.na(wsh20gf)) & (wsh20ge %in% c(1:4) | wsh20gf %in% c(1:4)) ~ 0),
      #for dis any 2
      wnhear2 = case_when (wsh20ge %in% c(2,3,4) | wsh20gf %in% c(2, 3,4) ~ 1,
                          (!is.na(wsh20ge) | !is.na(wsh20gf)) & (wsh20ge %in% c(1:4) | wsh20gf %in% c(1:4))  ~ 0),  
      
      #any disability
      wdis_any = ifelse (wnsee == 1 |wnhear == 1 | wsh20gg %in% c(3,4)| wsh20gh %in% c(3,4)| wsh20gi %in% c(3,4)| wsh20gj %in% c(3,4), "Yes", "No"),
                wdis_any=factor(wdis_any,levels=c("No","Yes")),
      
      #any disability - some, a lot, can't at all
      wdis_any2 = ifelse (wnsee2 == 1 |wnhear2 == 1 | wsh20gg %in% c(2,3,4)| wsh20gh %in% c(2,3,4)| wsh20gi %in% c(2,3,4)| wsh20gj %in% c(2,3,4), "Yes", "No"),
                wdis_any2=factor(wdis_any2,levels=c("No","Yes")),
      
      #disability spectrum variable - collapsed some+a lot
      wdis_sp1=case_when(wnsee==1 & wnhear==1 & wsh20gg==1 & wsh20gh==1 & wsh20gi==1 & wsh20gj==1 ~ 0,
                        wnsee==4 | wnhear==4 | wsh20gg==4 | wsh20gh==4 | wsh20gi==4 | wsh20gj==4 ~ 2,
                        wnsee %in% c(2,3) |wnhear %in% c(2,3) | wsh20gg %in% c(2,3)| wsh20gh %in% c(2,3)| wsh20gi %in% c(2,3)| wsh20gj %in% c(2,3) ~1,
                        TRUE ~ 0
      ),
      #disability spectrum variable - collapsed a lot+cannot
      wdis_sp2=case_when(wnsee==1 & wnhear==1 & wsh20gg==1 & wsh20gh==1 & wsh20gi==1 & wsh20gj==1 ~ 0,
                        wnsee %in% c(3,4) |wnhear %in% c(3,4) | wsh20gg %in% c(3,4)| wsh20gh %in% c(3,4)| wsh20gi %in% c(3,4)| wsh20gj %in% c(3,4) ~ 2,
                        wnsee==2 | wnhear==2 | wsh20gg==2 | wsh20gh==2 | wsh20gi==2 | wsh20gj==2 ~ 1,
                        TRUE ~ 0),
      #number of disabilities
      wdis_num=(wnsee %in% c(3,4))*1+(wnhear %in% c(3,4))*1+(wsh20gg %in% c(3,4))*1+(wsh20gh %in% c(3,4))*1+(wsh20gi %in% c(3,4))*1+(wsh20gj %in% c(3,4))*1,
      ...
      wreligion = case_when(wv130==97 ~ "No religion",
                            wv130==1 ~ "Catholic",
                            wv130==2 ~ "Protestant",
                            wv130==3 ~ "Adventist",
                            wv130==4 ~ "Muslim",
                            wv130==8 ~ "Jehovah Witness",
                            TRUE ~ "Other"),
                wreligion = factor(wreligion,levels=c("No religion","Catholic","Protestant","Adventist",
                                                      "Muslim","Jehovah Witness","Other")),
...
SNm <- mutate(SNm,
      ...
      # Disability
      #for dis any
      mnsee = case_when (sh20gb %in% c(3,4) | sh20gc %in% c(3,4) ~ 1,
                        (!is.na(sh20gb) | !is.na(sh20gc)) & (sh20gb %in% c(1:4) | sh20gc %in% c(1:4))  ~ 0),
      #for dis any 2
      mnsee2 = case_when (sh20gb %in% c(2,3,4) | sh20gc %in% c(2, 3,4) ~1,
                         (!is.na(sh20gb) | !is.na(sh20gc)) & (sh20gb %in% c(1:4) | sh20gc %in% c(1:4)) ~ 0),
      
      #for dis any
      mnhear = case_when (sh20ge %in% c(3,4) | sh20gf %in% c(3,4) ~ 1, 
                         (!is.na(sh20ge) | !is.na(sh20gf)) & (sh20ge %in% c(1:4) | sh20gf %in% c(1:4)) ~ 0),
      #for dis any 2
      mnhear2 = case_when (sh20ge %in% c(2,3,4) | sh20gf %in% c(2, 3,4) ~ 1,
                          (!is.na(sh20ge) | !is.na(sh20gf)) & (sh20ge %in% c(1:4) | sh20gf %in% c(1:4))  ~ 0),  
      
      #any disability
      mdis_any = ifelse (mnsee == 1 |mnhear == 1 | sh20gg %in% c(3,4)| sh20gh %in% c(3,4)| sh20gi %in% c(3,4)| sh20gj %in% c(3,4), "Yes", "No"),
                mdis_any=factor(mdis_any,levels=c("No","Yes")),
      
      #any disability - some, a lot, can't at all
      mdis_any2 = ifelse (mnsee2 == 1 |mnhear2 == 1 | sh20gg %in% c(2,3,4)| sh20gh %in% c(2,3,4)| sh20gi %in% c(2,3,4)| sh20gj %in% c(2,3,4), "Yes", "No"),
                mdis_any2=factor(mdis_any2,levels=c("No","Yes")),
      
      #disability spectrum variable - collapsed some+a lot
      mdis_sp1=case_when(mnsee==1 & mnhear==1 & sh20gg==1 & sh20gh==1 & sh20gi==1 & sh20gj==1 ~ 0,
                        mnsee==4 | mnhear==4 | sh20gg==4 | sh20gh==4 | sh20gi==4 | sh20gj==4 ~ 2,
                        mnsee %in% c(2,3) |mnhear %in% c(2,3) | sh20gg %in% c(2,3)| sh20gh %in% c(2,3)| sh20gi %in% c(2,3)| sh20gj %in% c(2,3) ~1,
                        TRUE ~ 0
      ),
      #disability spectrum variable - collapsed a lot+cannot
      mdis_sp2=case_when(mnsee==1 & mnhear==1 & sh20gg==1 & sh20gh==1 & sh20gi==1 & sh20gj==1 ~ 0,
                        mnsee %in% c(3,4) |mnhear %in% c(3,4) | sh20gg %in% c(3,4)| sh20gh %in% c(3,4)| sh20gi %in% c(3,4)| sh20gj %in% c(3,4) ~ 2,
                        mnsee==2 | mnhear==2 | sh20gg==2 | sh20gh==2 | sh20gi==2 | sh20gj==2 ~ 1,
                        TRUE ~ 0),
      #number of disabilities
      mdis_num=(mnsee %in% c(3,4))*1+(mnhear %in% c(3,4))*1+(sh20gg %in% c(3,4))*1+(sh20gh %in% c(3,4))*1+(sh20gi %in% c(3,4))*1+(sh20gj %in% c(3,4))*1,
      ...
      mliteracy = case_when(mv155==0 ~ "Cannot read at all",
                ...
                mliteracy = factor(mliteracy,levels = c("Cannot read at all",
                                  "Able to read only parts of sentence","Able to read whole sentence",
                                  "No card with required language","Blind/visually impaired")),
      mreligion = case_when(mv130==97 ~ "No religion",
                            mv130==1 ~ "Catholic",
                            mv130==2 ~ "Protestant",
                            mv130==3 ~ "Adventist",
                            mv130==4 ~ "Muslim",
                            mv130==8 ~ "Jehovah Witness",
                            TRUE ~ "Other"),
                mreligion = factor(mreligion,levels=c("No religion","Catholic","Protestant","Adventist",
                                                      "Muslim","Jehovah Witness","Other")),
...
# delete duplicates
SNm <- filter(SNm,mcaseid!=mlagid1|is.na(mlagid1))
# group_by(SNm,mcaseid) %>% summarise(n=n()) %>% filter(n>1) %>% group_by(n) %>%summarise(n2=n())
```
...
# group_by(SNm,whvidx==mv034_1,whvidx==mv034_2,whvidx==mv034_3,whvidx==mv034_4) %>% summarise(n=n())
# filter(SNm,is.na(whvidx == mv034_1)) %>% group_by(mv034_1,whvidx,hvidx) %>% summarise(n=n())
...
# test <- data.frame(table(SNm$mcaseid))
...
# for (name in names(SNm))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("SNm    ",name))
#   print(data.frame(group_by(SNm,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# for (name in names(SNw))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("SNw    ",name))
#   print(data.frame(group_by(SNw,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# sink()
```
...
```{r}
SNwa <- filter(SNw,!is.na(wnsee) & !is.na(wnhear) & wsh20gg!=8 & wsh20gh!=8 & wsh20gi!=8 & wsh20gj!=8) %>%
          filter(wv744a!=8 & wv744b!=8 & wv744c!=8 & wv744d!=8 & wv744e!=8)

SNma <- filter(SNm,!is.na(mnsee) & !is.na(mnhear) & sh20gg!=8 & sh20gh!=8 & sh20gi!=8 & sh20gj!=8) %>%
          filter(!is.na(wnsee) & !is.na(wnhear) & wsh20gg!=8 & wsh20gh!=8 & wsh20gi!=8 & wsh20gj!=8) %>%
...
SNwa <-mutate(SNwa, wt=wv005/1000000)
SNma <-mutate(SNma, wt=mv005/1000000)


wdesg <- svydesign(id=~wv021, strata=~wv023, weights=~wt, nest = TRUE, data = SNwa) 
mdesg <- svydesign(id=~mv021, strata=~mv023, weights=~wt, nest = TRUE, data = SNma) 


save(SNwa,SNma,file="SNa.Rdata")
...
for (out in wout)
{
  tab1 <- group_by(SNwa,get(out)) %>% summarise(n=n())
...
for (out in mout)
{
  tab1 <- group_by(SNma,get(out)) %>% summarise(n=n())
  tab1$percent <- tab1$n/sum(tab1$n)*100
  tab1$print <- paste0(tab1$n," (",round(tab1$percent,2),"%)")
  write.table(out,file="test.csv",append=T,sep=",")
  write.table(tab1,file="test.csv",append=T,sep=",",row.names=F,col.names=T)
...
{
  print(with(SNw,table(get(paste0("wv744",char)))))
}

for (num in c(1:4))
{
  print(with(SNm,table(get(paste0("mv034_",num))==whvidx)))
}

SNw$wjust <- SNw$wv744a==1|SNw$wv744b==1|SNw$wv744c==1|SNw$wv744d==1|SNw$wv744e==1

SNm2 <- SNm %>% filter(wv744a!=8&wv744b!=8&wv744c!=8&wv744d!=8&wv744e!=8&mv744a!=8&mv744b!=8&mv744c!=8&mv744d!=8&mv744e!=8)
SN3<- SNm  %>% select(hdis1, hdis2, hdis3, hdis4, hdis5,hdis6, hdis7, hdis8)
md.pattern(SN3, rotate.names = TRUE)
...
mwifeid <- unique(filter(SNm,mwifes=="Yes") %>% select(mcaseid))$mcaseid
# get if any of the wifes disabled, store in mwifevar
wda <- c()
wda2 <- c()
for (i in c(1:length(mwifeid)))
{
  test <- filter(SNm,mcaseid==mwifeid[i])
...
for (i in c(1:length(mwifeid)))
{
  SNm$wdis_any[SNm$mcaseid==mwifeid[i]] <- mwifevar[[i,2]]
  SNm$wdis_any2[SNm$mcaseid==mwifeid[i]] <- mwifevar[[i,3]]
}
# delete duplicates
SNm <- filter(SNm,mcaseid!=mlagid1)
```