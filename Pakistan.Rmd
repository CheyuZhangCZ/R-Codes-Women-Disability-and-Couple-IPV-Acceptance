...
mergepr <- PR %>% select(hv001,hv002,hvidx,num_range("hdis", 1:8),
                         hv105,hv025,hv270,hv207,hv208)
mergeir <- IR %>% select(caseid,v001,v002,v003,v034,v005,sv005,d005,
                         v021,v023,v024,
...
mergemr <- MR %>% select(mcaseid,mv001,mv002,mv003,mv005,smv005,mv021,mv023,
...
PKw <- mutate(PKw,
      ...
      # Disability
      #for dis any
      wnsee = case_when (whdis2 %in% c(3,4)  ~ 1,
                        !is.na(whdis2)  & whdis2 %in% c(1:4)   ~ 0),
      #for dis any 2
      wnsee2 = case_when (whdis2 %in% c(2,3,4)  ~1,
                         !is.na(whdis2)  & whdis2 %in% c(1:4)  ~ 0),
      
      #for dis any
      wnhear = case_when (whdis4 %in% c(3,4)  ~ 1,
                        !is.na(whdis4)  & whdis4 %in% c(1:4)   ~ 0),
      #for dis any 2
      wnhear2 = case_when (whdis4 %in% c(2,3,4)  ~1,
                         !is.na(whdis4)  & whdis4 %in% c(1:4)  ~ 0),
      
      #any disability
      wdis_any = ifelse (wnsee == 1 |wnhear == 1 | whdis5 %in% c(3,4)| whdis6 %in% c(3,4)| whdis7 %in% c(3,4)| whdis8 %in% c(3,4), "Yes", "No"),
                wdis_any=factor(wdis_any,levels=c("No","Yes")),
      
      #any disability - some, a lot, can't at all
      wdis_any2 = ifelse (wnsee2 == 1 |wnhear2 == 1 | whdis5 %in% c(2,3,4)| whdis6 %in% c(2,3,4)| whdis7 %in% c(2,3,4)| whdis8 %in% c(2,3,4), "Yes", "No"),
                wdis_any2=factor(wdis_any2,levels=c("No","Yes")),
      
      #disability spectrum variable - collapsed some+a lot
      wdis_sp1=case_when(wnsee==1 & wnhear==1 & whdis5==1 & whdis6==1 & whdis7==1 & whdis8==1 ~ 0,
                        wnsee==4 | wnhear==4 | whdis5==4 | whdis6==4 | whdis7==4 | whdis8==4 ~ 2,
                        wnsee %in% c(2,3) |wnhear %in% c(2,3) | whdis5 %in% c(2,3)| whdis6 %in% c(2,3)| whdis7 %in% c(2,3)| whdis8 %in% c(2,3) ~1,
                        TRUE ~ 0
      ),
      #disability spectrum variable - collapsed a lot+cannot
      wdis_sp2=case_when(wnsee==1 & wnhear==1 & whdis5==1 & whdis6==1 & whdis7==1 & whdis8==1 ~ 0,
                        wnsee %in% c(3,4) |wnhear %in% c(3,4) | whdis5 %in% c(3,4)| whdis6 %in% c(3,4)| whdis7 %in% c(3,4)| whdis8 %in% c(3,4) ~ 2,
                        wnsee==2 | wnhear==2 | whdis5==2 | whdis6==2 | whdis7==2 | whdis8==2 ~ 1,
                        TRUE ~ 0),
      #number of disabilities
      wdis_num=(wnsee %in% c(3,4))*1+(wnhear %in% c(3,4))*1+(whdis5 %in% c(3,4))*1+(whdis6 %in% c(3,4))*1+(whdis7 %in% c(3,4))*1+(whdis8 %in% c(3,4))*1,
...
PKm <- mutate(PKm,
      ...
      # Disability
      #for dis any
      mnsee = case_when (hdis2 %in% c(3,4)  ~ 1,
                        !is.na(hdis2)  & hdis2 %in% c(1:4)   ~ 0),
      #for dis any 2
      mnsee2 = case_when (hdis2 %in% c(2,3,4)  ~1,
                         !is.na(hdis2)  & hdis2 %in% c(1:4)  ~ 0),
      
      #for dis any
      mnhear = case_when (hdis4 %in% c(3,4)  ~ 1,
                        !is.na(hdis4)  & hdis4 %in% c(1:4)   ~ 0),
      #for dis any 2
      mnhear2 = case_when (hdis4 %in% c(2,3,4)  ~1,
                         !is.na(hdis4)  & hdis4 %in% c(1:4)  ~ 0),    
      
      #any disability
      mdis_any = ifelse (mnsee == 1 |mnhear == 1 | hdis5 %in% c(3,4)| hdis6 %in% c(3,4)| hdis7 %in% c(3,4)| hdis8 %in% c(3,4), "Yes", "No"),
                mdis_any=factor(mdis_any,levels=c("No","Yes")),
      
      #any disability - some, a lot, can't at all
      mdis_any2 = ifelse (mnsee2 == 1 |mnhear2 == 1 | hdis5 %in% c(2,3,4)| hdis6 %in% c(2,3,4)| hdis7 %in% c(2,3,4)| hdis8 %in% c(2,3,4), "Yes", "No"),
                mdis_any2=factor(mdis_any2,levels=c("No","Yes")),
      
      #disability spectrum variable - collapsed some+a lot
      mdis_sp1=case_when(mnsee==1 & mnhear==1 & hdis5==1 & hdis6==1 & hdis7==1 & hdis8==1 ~ 0,
                        mnsee==4 | mnhear==4 | hdis5==4 | hdis6==4 | hdis7==4 | hdis8==4 ~ 2,
                        mnsee %in% c(2,3) |mnhear %in% c(2,3) | hdis5 %in% c(2,3)| hdis6 %in% c(2,3)| hdis7 %in% c(2,3)| hdis8 %in% c(2,3) ~1,
                        TRUE ~ 0
      ),
      #disability spectrum variable - collapsed a lot+cannot
      mdis_sp2=case_when(mnsee==1 & mnhear==1 & hdis5==1 & hdis6==1 & hdis7==1 & hdis8==1 ~ 0,
                        mnsee %in% c(3,4) |mnhear %in% c(3,4) | hdis5 %in% c(3,4)| hdis6 %in% c(3,4)| hdis7 %in% c(3,4)| hdis8 %in% c(3,4) ~ 2,
                        mnsee==2 | mnhear==2 | hdis5==2 | hdis6==2 | hdis7==2 | hdis8==2 ~ 1,
                        TRUE ~ 0),
      #number of disabilities
      mdis_num=(mnsee %in% c(3,4))*1+(mnhear %in% c(3,4))*1+(hdis5 %in% c(3,4))*1+(hdis6 %in% c(3,4))*1+(hdis7 %in% c(3,4))*1+(hdis8 %in% c(3,4))*1,
...
# delete duplicates
PKm <- filter(PKm,mcaseid!=mlagid1|is.na(mlagid1))
# group_by(PKm,mcaseid) %>% summarise(n=n()) %>% filter(n>1) %>% group_by(n) %>%summarise(n2=n())
```
...
# group_by(PKm,whvidx==mv034_1,whvidx==mv034_2,whvidx==mv034_3,whvidx==mv034_4) %>% summarise(n=n())
# filter(PKm,is.na(whvidx == mv034_1)) %>% group_by(mv034_1,whvidx,hvidx) %>% summarise(n=n())
...
# test <- data.frame(table(PKm$mcaseid))
...
# for (name in names(PKm))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("PKm    ",name))
#   print(data.frame(group_by(PKm,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# for (name in names(PKw))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("PKw    ",name))
#   print(data.frame(group_by(PKw,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# sink()
```
...
```{r}
PKwa <- filter(PKw,!is.na(wnsee) & !is.na(wnhear) & whdis5!=8 & whdis6!=8 & whdis7!=8 & whdis8!=8) %>%
          filter(wv744a!=8 & wv744b!=8 & wv744c!=8 & wv744d!=8 & wv744e!=8)

PKma <- filter(PKm,!is.na(mnsee) & !is.na(mnhear) & hdis5!=8 & hdis6!=8 & hdis7!=8 & hdis8!=8) %>%
          filter(!is.na(wnsee) & !is.na(wnhear) & whdis5!=8 & whdis6!=8 & whdis7!=8 & whdis8!=8) %>%
...
PKwa <-mutate(PKwa, wt=(wv005+wsv005)/1000000)
PKma <-mutate(PKma, wt=(mv005+smv005)/1000000)


wdesg <- svydesign(id=~wv021, strata=~wv023, weights=~wt, nest = TRUE, data = PKwa) 
mdesg <- svydesign(id=~mv021, strata=~mv023, weights=~wt, nest = TRUE, data = PKma) 


save(PKwa,PKma,file="PKa.Rdata")
...
wcov2 <- c("wage","wedu","wlivchild","wwork","wwealth","wurbrur","wliteracy",
          "wtvradio","wfreqmedia")
mcov <- c("mage","medu","mlivchild","mwork","mwealth","murbrur","mmarital","mliteracy",
          "mreligion","mtvradio","mfreqmedia","mpolygamy")
mcov2 <- c("mage","medu","mlivchild","mwork","mwealth","murbrur","mliteracy",
...
for (out in wout)
{
  tab1 <- group_by(PKwa,get(out)) %>% summarise(n=n())
...
for (out in mout)
{
  tab1 <- group_by(PKma,get(out)) %>% summarise(n=n())
  tab1$percent <- tab1$n/sum(tab1$n)*100
  tab1$print <- paste0(tab1$n," (",round(tab1$percent,2),"%)")
  write.table(out,file="test.csv",append=T,sep=",")
  write.table(tab1,file="test.csv",append=T,sep=",",row.names=F,col.names=T)
...
for(dis in c("wdis_any2"))
{
  for (out in c("wjust_any"))
  {
    ...
    for (cov in wcov2)
    {
      write.table(paste0("one adjustment ",out,"~",dis),file="test.csv",append=T,sep=",")
...
modelobject <- svyglm(mjust_any ~ wdis_any2 + mage+medu+mlivchild+mwork+mwealth+murbrur+
                        mliteracy+mtvradio+mfreqmedia+mpolygamy, 
...
modelobject <- svyglm(mjust_any ~  wdis_any2 + mdis_any2 + mage+medu+mlivchild+mwork+mwealth+murbrur+
               ...
               design = mdesg, family = quasibinomial(link = "logit"))
modelobject <- svyglm(mjust_any ~  wdis_any2 + mdis_any2 + mage+medu+mlivchild+mwork+mwealth+murbrur+
                        mliteracy+mtvradio+mfreqmedia+mpolygamy + wdis_any2*mdis_any2, 
               design = mdesg, family = quasibinomial(link = "logit"))
AIC(modelobject)
summary(modelobject)
printer(modelobject)
...
{
  print(with(PKw,table(get(paste0("wv744",char)))))
}

for (num in c(1:4))
{
  print(with(PKm,table(get(paste0("mv034_",num))==whvidx)))
}

PKw$wjust <- PKw$wv744a==1|PKw$wv744b==1|PKw$wv744c==1|PKw$wv744d==1|PKw$wv744e==1

PKm2 <- PKm %>% filter(wv744a!=8&wv744b!=8&wv744c!=8&wv744d!=8&wv744e!=8&mv744a!=8&mv744b!=8&mv744c!=8&mv744d!=8&mv744e!=8)
PK3<- PKm  %>% select(hdis1, hdis2, hdis3, hdis4, hdis5,hdis6, hdis7, hdis8)
md.pattern(PK3, rotate.names = TRUE)
...
mwifeid <- unique(filter(PKm,mwifes=="Yes") %>% select(mcaseid))$mcaseid
# get if any of the wifes disabled, store in mwifevar
wda <- c()
wda2 <- c()
for (i in c(1:length(mwifeid)))
{
  test <- filter(PKm,mcaseid==mwifeid[i])
...
for (i in c(1:length(mwifeid)))
{
  PKm$wdis_any[PKm$mcaseid==mwifeid[i]] <- mwifevar[[i,2]]
  PKm$wdis_any2[PKm$mcaseid==mwifeid[i]] <- mwifevar[[i,3]]
}
# delete duplicates
PKm <- filter(PKm,mcaseid!=mlagid1)
```