...
# delete duplicates
MLm <- filter(MLm,mcaseid!=mlagid1|is.na(mlagid1))

# group_by(MLm,mcaseid) %>% summarise(n=n()) %>% filter(n>1) %>% group_by(n) %>%summarise(n2=n())
```
...
# group_by(MLm,whvidx==mv034_1,whvidx==mv034_2,whvidx==mv034_3,whvidx==mv034_4) %>% summarise(n=n())
# filter(MLm,is.na(whvidx == mv034_1)) %>% group_by(mv034_1,whvidx,hvidx) %>% summarise(n=n())
...
# test <- data.frame(table(MLm$mcaseid))
...
# for (name in names(MLm))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("MLm    ",name))
#   print(data.frame(group_by(MLm,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# for (name in names(MLw))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("MLw    ",name))
#   print(data.frame(group_by(MLw,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# sink()
```
...
```{r}
MLwa <- filter(MLw,!is.na(wnsee) & !is.na(wnhear) & whdis5!=8 & whdis6!=8 & whdis7!=8 & whdis8!=8) %>%
          filter(wv744a!=8 & wv744b!=8 & wv744c!=8 & wv744d!=8 & wv744e!=8)

MLma <- filter(MLm,!is.na(mnsee) & !is.na(mnhear) & hdis5!=8 & hdis6!=8 & hdis7!=8 & hdis8!=8) %>%
...
MLwa <-mutate(MLwa, wt=wv005/1000000)
MLma <-mutate(MLma, wt=mv005/1000000)


wdesg <- svydesign(id=~wv021, strata=~wv023, weights=~wt, nest = TRUE, data = MLwa) 
mdesg <- svydesign(id=~mv021, strata=~mv023, weights=~wt, nest = TRUE, data = MLma) 


save(MLwa,MLma,file="MLa.Rdata")
...
for (out in wout)
{
  tab1 <- group_by(MLwa,get(out)) %>% summarise(n=n())
...
for (out in mout)
{
  tab1 <- group_by(MLma,get(out)) %>% summarise(n=n())
  tab1$percent <- tab1$n/sum(tab1$n)*100
  tab1$print <- paste0(tab1$n," (",round(tab1$percent,2),"%)")
  write.table(out,file="test.csv",append=T,sep=",")
  write.table(tab1,file="test.csv",append=T,sep=",",row.names=F,col.names=T)
...
{
  print(with(MLw,table(get(paste0("wv744",char)))))
}

for (num in c(1:4))
{
  print(with(MLm,table(get(paste0("mv034_",num))==whvidx)))
}

MLw$wjust <- MLw$wv744a==1|MLw$wv744b==1|MLw$wv744c==1|MLw$wv744d==1|MLw$wv744e==1

MLm2 <- MLm %>% filter(wv744a!=8&wv744b!=8&wv744c!=8&wv744d!=8&wv744e!=8&mv744a!=8&mv744b!=8&mv744c!=8&mv744d!=8&mv744e!=8)
ML3<- MLm  %>% select(hdis1, hdis2, hdis3, hdis4, hdis5,hdis6, hdis7, hdis8)
md.pattern(ML3, rotate.names = TRUE)
...
mwifeid <- unique(filter(MLm,mwifes=="Yes") %>% select(mcaseid))$mcaseid
# get if any of the wifes disabled, store in mwifevar
wda <- c()
wda2 <- c()
for (i in c(1:length(mwifeid)))
{
  test <- filter(MLm,mcaseid==mwifeid[i])
...
for (i in c(1:length(mwifeid)))
{
  MLm$wdis_any[MLm$mcaseid==mwifeid[i]] <- mwifevar[[i,2]]
  MLm$wdis_any2[MLm$mcaseid==mwifeid[i]] <- mwifevar[[i,3]]
}
# delete duplicates
MLm <- filter(MLm,mcaseid!=mlagid1)
```