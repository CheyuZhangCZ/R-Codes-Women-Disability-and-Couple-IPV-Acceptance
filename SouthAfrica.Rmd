...
# delete duplicates
ZAm <- filter(ZAm,mcaseid!=mlagid1|is.na(mlagid1))
# group_by(ZAm,mcaseid) %>% summarise(n=n()) %>% filter(n>1) %>% group_by(n) %>%summarise(n2=n())
```
...
# group_by(ZAm,whvidx==mv034_1,whvidx==mv034_2,whvidx==mv034_3,whvidx==mv034_4) %>% summarise(n=n())
# filter(ZAm,is.na(whvidx == mv034_1)) %>% group_by(mv034_1,whvidx,hvidx) %>% summarise(n=n())
...
# test <- data.frame(table(ZAm$mcaseid))
...
# for (name in names(ZAm))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("ZAm    ",name))
#   print(data.frame(group_by(ZAm,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# for (name in names(ZAw))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("ZAw    ",name))
#   print(data.frame(group_by(ZAw,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# sink()
```
...
```{r}
ZAwa <- filter(ZAw,!is.na(wnsee) & !is.na(wnhear) & whdis5!=8 & whdis6!=8 & whdis7!=8 & whdis8!=8) %>%
          filter(wv744a!=8 & wv744b!=8 & wv744c!=8 & wv744d!=8 & wv744e!=8)

ZAma <- filter(ZAm,!is.na(mnsee) & !is.na(mnhear) & hdis5!=8 & hdis6!=8 & hdis7!=8 & hdis8!=8) %>%
...
ZAwa <-mutate(ZAwa, wt=wv005/1000000)
ZAma <-mutate(ZAma, wt=mv005/1000000)


wdesg <- svydesign(id=~wv021, strata=~wv023, weights=~wt, nest = TRUE, data = ZAwa) 
mdesg <- svydesign(id=~mv021, strata=~mv023, weights=~wt, nest = TRUE, data = ZAma) 


save(ZAwa,ZAma,file="ZAa.Rdata")
...
for (out in wout)
{
  tab1 <- group_by(ZAwa,get(out)) %>% summarise(n=n())
...
for (out in mout)
{
  tab1 <- group_by(ZAma,get(out)) %>% summarise(n=n())
  tab1$percent <- tab1$n/sum(tab1$n)*100
  tab1$print <- paste0(tab1$n," (",round(tab1$percent,2),"%)")
  write.table(out,file="test.csv",append=T,sep=",")
  write.table(tab1,file="test.csv",append=T,sep=",",row.names=F,col.names=T)
...
{
  print(with(ZAw,table(get(paste0("wv744",char)))))
}

for (num in c(1:4))
{
  print(with(ZAm,table(get(paste0("mv034_",num))==whvidx)))
}

ZAw$wjust <- ZAw$wv744a==1|ZAw$wv744b==1|ZAw$wv744c==1|ZAw$wv744d==1|ZAw$wv744e==1

ZAm2 <- ZAm %>% filter(wv744a!=8&wv744b!=8&wv744c!=8&wv744d!=8&wv744e!=8&mv744a!=8&mv744b!=8&mv744c!=8&mv744d!=8&mv744e!=8)
ZA3<- ZAm  %>% select(hdis1, hdis2, hdis3, hdis4, hdis5,hdis6, hdis7, hdis8)
md.pattern(ZA3, rotate.names = TRUE)
...
mwifeid <- unique(filter(ZAm,mwifes=="Yes") %>% select(mcaseid))$mcaseid
# get if any of the wifes disabled, store in mwifevar
wda <- c()
wda2 <- c()
for (i in c(1:length(mwifeid)))
{
  test <- filter(ZAm,mcaseid==mwifeid[i])
...
for (i in c(1:length(mwifeid)))
{
  ZAm$wdis_any[ZAm$mcaseid==mwifeid[i]] <- mwifevar[[i,2]]
  ZAm$wdis_any2[ZAm$mcaseid==mwifeid[i]] <- mwifevar[[i,3]]
}
# delete duplicates
ZAm <- filter(ZAm,mcaseid!=mlagid1)
```