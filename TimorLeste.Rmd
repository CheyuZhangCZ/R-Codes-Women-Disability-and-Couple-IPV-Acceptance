...
TLw <- mutate(TLw,
      ...
      wreligion = case_when(wv130==1 ~ "Roman Catholic",
                            wv130==2 ~ "Muslim",
                            wv130==3 ~ "Protestant",
                            wv130==4 ~ "Hindu",
                            TRUE ~ "Other"),
                wreligion = factor(wreligion,levels=c("Roman Catholic","Muslim","Protestant",
                                                      "Hindu","Other")),
...
TLm <- mutate(TLm,
      ...
      mliteracy = case_when(mv155==0 ~ "Cannot read at all",
                ...
                mliteracy = factor(mliteracy,levels = c("Cannot read at all",
                                  "Able to read only parts of sentence","Able to read whole sentence",
                                  "No card with required language","Blind/visually impaired")),
      mreligion = case_when(mv130==1 ~ "Roman Catholic",
                            mv130==2 ~ "Muslim",
                            mv130==3 ~ "Protestant",
                            mv130==4 ~ "Hindu",
                            TRUE ~ "Other"),
                mreligion = factor(mreligion,levels=c("Roman Catholic","Muslim","Protestant",
                                                      "Hindu","Other")),
...
# delete duplicates
TLm <- filter(TLm,mcaseid!=mlagid1|is.na(mlagid1))
# group_by(TLm,mcaseid) %>% summarise(n=n()) %>% filter(n>1) %>% group_by(n) %>%summarise(n2=n())
```
...
# group_by(TLm,whvidx==mv034_1,whvidx==mv034_2,whvidx==mv034_3,whvidx==mv034_4) %>% summarise(n=n())
# filter(TLm,is.na(whvidx == mv034_1)) %>% group_by(mv034_1,whvidx,hvidx) %>% summarise(n=n())
...
# test <- data.frame(table(TLm$mcaseid))
...
# for (name in names(TLm))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("TLm    ",name))
#   print(data.frame(group_by(TLm,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# for (name in names(TLw))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("TLw    ",name))
#   print(data.frame(group_by(TLw,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# sink()
```
...
```{r}
TLwa <- filter(TLw,!is.na(wnsee) & !is.na(wnhear) & whdis5!=8 & whdis6!=8 & whdis7!=8 & whdis8!=8) %>%
          filter(wv744a!=8 & wv744b!=8 & wv744c!=8 & wv744d!=8 & wv744e!=8)

TLma <- filter(TLm,!is.na(mnsee) & !is.na(mnhear) & hdis5!=8 & hdis6!=8 & hdis7!=8 & hdis8!=8) %>%
...
TLwa <-mutate(TLwa, wt=wv005/1000000)
TLma <-mutate(TLma, wt=mv005/1000000)


wdesg <- svydesign(id=~wv021, strata=~wv023, weights=~wt, nest = TRUE, data = TLwa) 
mdesg <- svydesign(id=~mv021, strata=~mv023, weights=~wt, nest = TRUE, data = TLma) 


save(TLwa,TLma,file="TLa.Rdata")
...
for (out in wout)
{
  tab1 <- group_by(TLwa,get(out)) %>% summarise(n=n())
...
for (out in mout)
{
  tab1 <- group_by(TLma,get(out)) %>% summarise(n=n())
  tab1$percent <- tab1$n/sum(tab1$n)*100
  tab1$print <- paste0(tab1$n," (",round(tab1$percent,2),"%)")
  write.table(out,file="test.csv",append=T,sep=",")
  write.table(tab1,file="test.csv",append=T,sep=",",row.names=F,col.names=T)
...
{
  print(with(TLw,table(get(paste0("wv744",char)))))
}

for (num in c(1:4))
{
  print(with(TLm,table(get(paste0("mv034_",num))==whvidx)))
}

TLw$wjust <- TLw$wv744a==1|TLw$wv744b==1|TLw$wv744c==1|TLw$wv744d==1|TLw$wv744e==1

TLm2 <- TLm %>% filter(wv744a!=8&wv744b!=8&wv744c!=8&wv744d!=8&wv744e!=8&mv744a!=8&mv744b!=8&mv744c!=8&mv744d!=8&mv744e!=8)
TL3<- TLm  %>% select(hdis1, hdis2, hdis3, hdis4, hdis5,hdis6, hdis7, hdis8)
md.pattern(TL3, rotate.names = TRUE)
...
mwifeid <- unique(filter(TLm,mwifes=="Yes") %>% select(mcaseid))$mcaseid
# get if any of the wifes disabled, store in mwifevar
wda <- c()
wda2 <- c()
for (i in c(1:length(mwifeid)))
{
  test <- filter(TLm,mcaseid==mwifeid[i])
...
for (i in c(1:length(mwifeid)))
{
  TLm$wdis_any[TLm$mcaseid==mwifeid[i]] <- mwifevar[[i,2]]
  TLm$wdis_any2[TLm$mcaseid==mwifeid[i]] <- mwifevar[[i,3]]
}
# delete duplicates
TLm <- filter(TLm,mcaseid!=mlagid1)
```