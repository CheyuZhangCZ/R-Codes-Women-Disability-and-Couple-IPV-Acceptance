...
# delete duplicates
RWm <- filter(RWm,mcaseid!=mlagid1|is.na(mlagid1))
# group_by(RWm,mcaseid) %>% summarise(n=n()) %>% filter(n>1) %>% group_by(n) %>%summarise(n2=n())
```
...
# group_by(RWm,whvidx==mv034_1,whvidx==mv034_2,whvidx==mv034_3,whvidx==mv034_4) %>% summarise(n=n())
# filter(RWm,is.na(whvidx == mv034_1)) %>% group_by(mv034_1,whvidx,hvidx) %>% summarise(n=n())
...
# test <- data.frame(table(RWm$mcaseid))
...
# for (name in names(RWm))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("RWm    ",name))
#   print(data.frame(group_by(RWm,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# for (name in names(RWw))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("RWw    ",name))
#   print(data.frame(group_by(RWw,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# sink()
```
...
```{r}
RWwa <- filter(RWw,!is.na(wnsee) & !is.na(wnhear) & whdis5!=8 & whdis6!=8 & whdis7!=8 & whdis8!=8) %>%
          filter(wv744a!=8 & wv744b!=8 & wv744c!=8 & wv744d!=8 & wv744e!=8)

RWma <- filter(RWm,!is.na(mnsee) & !is.na(mnhear) & hdis5!=8 & hdis6!=8 & hdis7!=8 & hdis8!=8) %>%
...
RWwa <-mutate(RWwa, wt=wv005/1000000)
RWma <-mutate(RWma, wt=mv005/1000000)


wdesg <- svydesign(id=~wv021, strata=~wv023, weights=~wt, nest = TRUE, data = RWwa) 
mdesg <- svydesign(id=~mv021, strata=~mv023, weights=~wt, nest = TRUE, data = RWma) 


save(RWwa,RWma,file="RWa.Rdata")
...
for (out in wout)
{
  tab1 <- group_by(RWwa,get(out)) %>% summarise(n=n())
...
for (out in mout)
{
  tab1 <- group_by(RWma,get(out)) %>% summarise(n=n())
  tab1$percent <- tab1$n/sum(tab1$n)*100
  tab1$print <- paste0(tab1$n," (",round(tab1$percent,2),"%)")
  write.table(out,file="test.csv",append=T,sep=",")
  write.table(tab1,file="test.csv",append=T,sep=",",row.names=F,col.names=T)
...
{
  print(with(RWw,table(get(paste0("wv744",char)))))
}

for (num in c(1:4))
{
  print(with(RWm,table(get(paste0("mv034_",num))==whvidx)))
}

RWw$wjust <- RWw$wv744a==1|RWw$wv744b==1|RWw$wv744c==1|RWw$wv744d==1|RWw$wv744e==1

RWm2 <- RWm %>% filter(wv744a!=8&wv744b!=8&wv744c!=8&wv744d!=8&wv744e!=8&mv744a!=8&mv744b!=8&mv744c!=8&mv744d!=8&mv744e!=8)
RW3<- RWm  %>% select(hdis1, hdis2, hdis3, hdis4, hdis5,hdis6, hdis7, hdis8)
md.pattern(RW3, rotate.names = TRUE)
...
mwifeid <- unique(filter(RWm,mwifes=="Yes") %>% select(mcaseid))$mcaseid
# get if any of the wifes disabled, store in mwifevar
wda <- c()
wda2 <- c()
for (i in c(1:length(mwifeid)))
{
  test <- filter(RWm,mcaseid==mwifeid[i])
...
for (i in c(1:length(mwifeid)))
{
  RWm$wdis_any[RWm$mcaseid==mwifeid[i]] <- mwifevar[[i,2]]
  RWm$wdis_any2[RWm$mcaseid==mwifeid[i]] <- mwifevar[[i,3]]
}
# delete duplicates
RWm <- filter(RWm,mcaseid!=mlagid1)
```