...
for (sn in shortnames) ##UG
{
  ...
  mergepr <- PR %>% select(hv001,hv002,hvidx,num_range("sh", 23:32),
  ...
  mergeir <- IR %>% select(caseid,v001,v002,v003,v034,v005,d005,
                           ...
                           d106,d107,d108,
                           v106,v501,v502,v511,v218,v716,v130,v155,v739,v157,v158,v159,v171a,v171b)
  mergemr <- MR %>% select(mcaseid,mv001,mv002,mv003,mv005,mv021,mv023,
                           ...
                           mv130,mv155,mv739,mv157,mv158,mv159,mv171a,mv171b)
...
UGw <- mutate(UGw,
      ...
      # Disability
      #for dis any
      wnsee = case_when (wsh24 %in% c(3,4) | wsh25 %in% c(3,4) ~ 1,
                        (!is.na(wsh24) | !is.na(wsh25)) & (wsh24 %in% c(1:4) | wsh25 %in% c(1:4))  ~ 0),
      #for dis any 2
      wnsee2 = case_when (wsh24 %in% c(2,3,4) | wsh25 %in% c(2, 3,4) ~1,
                         (!is.na(wsh24) | !is.na(wsh25)) & (wsh24 %in% c(1:4) | wsh25 %in% c(1:4)) ~ 0),
      
      #for dis any
      wnhear = case_when (wsh27 %in% c(3,4) | wsh28 %in% c(3,4) ~ 1, 
                         (!is.na(wsh27) | !is.na(wsh28)) & (wsh27 %in% c(1:4) | wsh28 %in% c(1:4)) ~ 0),
      #for dis any 2
      wnhear2 = case_when (wsh27 %in% c(2,3,4) | wsh28 %in% c(2, 3,4) ~ 1,
                          (!is.na(wsh27) | !is.na(wsh28)) & (wsh27 %in% c(1:4) | wsh28 %in% c(1:4))  ~ 0),  
      
      #any disability
      wdis_any = ifelse (wnsee == 1 |wnhear == 1 | wsh29 %in% c(3,4)| wsh30 %in% c(3,4)| wsh31 %in% c(3,4)| wsh32 %in% c(3,4), "Yes", "No"),
                wdis_any=factor(wdis_any,levels=c("No","Yes")),
      
      #any disability - some, a lot, can't at all
      wdis_any2 = ifelse (wnsee2 == 1 |wnhear2 == 1 | wsh29 %in% c(2,3,4)| wsh30 %in% c(2,3,4)| wsh31 %in% c(2,3,4)| wsh32 %in% c(2,3,4), "Yes", "No"),
                wdis_any2=factor(wdis_any2,levels=c("No","Yes")),
      
      #disability spectrum variable - collapsed some+a lot
      wdis_sp1=case_when(wnsee==1 & wnhear==1 & wsh29==1 & wsh30==1 & wsh31==1 & wsh32==1 ~ 0,
                        wnsee==4 | wnhear==4 | wsh29==4 | wsh30==4 | wsh31==4 | wsh32==4 ~ 2,
                        wnsee %in% c(2,3) |wnhear %in% c(2,3) | wsh29 %in% c(2,3)| wsh30 %in% c(2,3)| wsh31 %in% c(2,3)| wsh32 %in% c(2,3) ~1,
                        TRUE ~ 0
      ),
      #disability spectrum variable - collapsed a lot+cannot
      wdis_sp2=case_when(wnsee==1 & wnhear==1 & wsh29==1 & wsh30==1 & wsh31==1 & wsh32==1 ~ 0,
                        wnsee %in% c(3,4) |wnhear %in% c(3,4) | wsh29 %in% c(3,4)| wsh30 %in% c(3,4)| wsh31 %in% c(3,4)| wsh32 %in% c(3,4) ~ 2,
                        wnsee==2 | wnhear==2 | wsh29==2 | wsh30==2 | wsh31==2 | wsh32==2 ~ 1,
                        TRUE ~ 0),
      #number of disabilities
      wdis_num=(wnsee %in% c(3,4))*1+(wnhear %in% c(3,4))*1+(wsh29 %in% c(3,4))*1+(wsh30 %in% c(3,4))*1+(wsh31 %in% c(3,4))*1+(wsh32 %in% c(3,4))*1,
      ...
      wreligion = case_when(wv130==0 ~ "No religion",
                            wv130==1 ~ "Anglican",
                            wv130==2 ~ "Catholic",
                            wv130==3 ~ "Muslim",
                            wv130==6 ~ "Pentecostal / Born again / Evangelical",
                            TRUE ~ "Other"),
                wreligion = factor(wreligion,levels=c("No religion","Anglican","Catholic","Muslim",
                                                      "Pentecostal / Born again / Evangelical","Other")),
      ...
      winternet = case_when(wv171b==0 ~ "Not at all",
                            wv171b==1 ~ "Less than once a week",
                            wv171b==2 ~ "At least once a week",
                            wv171b==3 ~ "Almost every day"),
...
UGm <- mutate(UGm,
      ...
      # Disability
      #for dis any
      mnsee = case_when (sh24 %in% c(3,4) | sh25 %in% c(3,4) ~ 1,
                        (!is.na(sh24) | !is.na(sh25)) & (sh24 %in% c(1:4) | sh25 %in% c(1:4))  ~ 0),
      #for dis any 2
      mnsee2 = case_when (sh24 %in% c(2,3,4) | sh25 %in% c(2, 3,4) ~1,
                         (!is.na(sh24) | !is.na(sh25)) & (sh24 %in% c(1:4) | sh25 %in% c(1:4)) ~ 0),
      
      #for dis any
      mnhear = case_when (sh27 %in% c(3,4) | sh28 %in% c(3,4) ~ 1, 
                         (!is.na(sh27) | !is.na(sh28)) & (sh27 %in% c(1:4) | sh28 %in% c(1:4)) ~ 0),
      #for dis any 2
      mnhear2 = case_when (sh27 %in% c(2,3,4) | sh28 %in% c(2, 3,4) ~ 1,
                          (!is.na(sh27) | !is.na(sh28)) & (sh27 %in% c(1:4) | sh28 %in% c(1:4))  ~ 0),  
      
      #any disability
      mdis_any = ifelse (mnsee == 1 |mnhear == 1 | sh29 %in% c(3,4)| sh30 %in% c(3,4)| sh31 %in% c(3,4)| sh32 %in% c(3,4), "Yes", "No"),
                mdis_any=factor(mdis_any,levels=c("No","Yes")),
      
      #any disability - some, a lot, can't at all
      mdis_any2 = ifelse (mnsee2 == 1 |mnhear2 == 1 | sh29 %in% c(2,3,4)| sh30 %in% c(2,3,4)| sh31 %in% c(2,3,4)| sh32 %in% c(2,3,4), "Yes", "No"),
                mdis_any2=factor(mdis_any2,levels=c("No","Yes")),
      
      #disability spectrum variable - collapsed some+a lot
      mdis_sp1=case_when(mnsee==1 & mnhear==1 & sh29==1 & sh30==1 & sh31==1 & sh32==1 ~ 0,
                        mnsee==4 | mnhear==4 | sh29==4 | sh30==4 | sh31==4 | sh32==4 ~ 2,
                        mnsee %in% c(2,3) |mnhear %in% c(2,3) | sh29 %in% c(2,3)| sh30 %in% c(2,3)| sh31 %in% c(2,3)| sh32 %in% c(2,3) ~1,
                        TRUE ~ 0
      ),
      #disability spectrum variable - collapsed a lot+cannot
      mdis_sp2=case_when(mnsee==1 & mnhear==1 & sh29==1 & sh30==1 & sh31==1 & sh32==1 ~ 0,
                        mnsee %in% c(3,4) |mnhear %in% c(3,4) | sh29 %in% c(3,4)| sh30 %in% c(3,4)| sh31 %in% c(3,4)| sh32 %in% c(3,4) ~ 2,
                        mnsee==2 | mnhear==2 | sh29==2 | sh30==2 | sh31==2 | sh32==2 ~ 1,
                        TRUE ~ 0),
      #number of disabilities
      mdis_num=(mnsee %in% c(3,4))*1+(mnhear %in% c(3,4))*1+(sh29 %in% c(3,4))*1+(sh30 %in% c(3,4))*1+(sh31 %in% c(3,4))*1+(sh32 %in% c(3,4))*1,
      ...
      mliteracy = case_when(mv155==0 ~ "Cannot read at all",
                ...
                mliteracy = factor(mliteracy,levels = c("Cannot read at all",
                                  "Able to read only parts of sentence","Able to read whole sentence",
                                  "No card with required language","Blind/visually impaired")),
      mreligion = case_when(mv130==0 ~ "No religion",
                            mv130==1 ~ "Anglican",
                            mv130==2 ~ "Catholic",
                            mv130==3 ~ "Muslim",
                            mv130==6 ~ "Pentecostal / Born again / Evangelical",
                            TRUE ~ "Other"),
                mreligion = factor(mreligion,levels=c("No religion","Anglican","Catholic","Muslim",
                                                      "Pentecostal / Born again / Evangelical","Other")),
      ...
      mfreqmedia = case_when(mv157==2 | mv158==2 | mv159==2 ~ "At least once a week",
                ...
                mfreqmedia = factor(mfreqmedia,levels=c("Not at all","Less than once a week",
                                                        "At least once a week")),
      minternet = case_when(mv171b==0 ~ "Not at all",
                            mv171b==1 ~ "Less than once a week",
                            mv171b==2 ~ "At least once a week",
                            mv171b==3 ~ "Almost every day"),
...
# delete duplicates
UGm <- filter(UGm,mcaseid!=mlagid1|is.na(mlagid1))
# group_by(UGm,mcaseid) %>% summarise(n=n()) %>% filter(n>1) %>% group_by(n) %>%summarise(n2=n())
```
...
# group_by(UGm,whvidx==mv034_1,whvidx==mv034_2,whvidx==mv034_3,whvidx==mv034_4) %>% summarise(n=n())
# filter(UGm,is.na(whvidx == mv034_1)) %>% group_by(mv034_1,whvidx,hvidx) %>% summarise(n=n())
...
# test <- data.frame(table(UGm$mcaseid))
...
# for (name in names(UGm))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("UGm    ",name))
#   print(data.frame(group_by(UGm,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# for (name in names(UGw))
# {
#   if (name %in% c("hv001","whv001","hv002","whv002","hvidx","whvidx","wv034","wd005","wv005","mv005",
#                   "wv021","wv023","mv021","mv023","wcaseid","mcaseid"))
#     {next}
#   print(paste0("UGw    ",name))
#   print(data.frame(group_by(UGw,get(name)) %>% summarise(n=n())))
#   writeLines("\n")
# }
# sink()
```
...
```{r}
UGwa <- filter(UGw,!is.na(wnsee) & !is.na(wnhear) & wsh29!=8 & wsh30!=8 & wsh31!=8 & wsh32!=8) %>%
          filter(wv744a!=8 & wv744b!=8 & wv744c!=8 & wv744d!=8 & wv744e!=8)

UGma <- filter(UGm,!is.na(mnsee) & !is.na(mnhear) & sh29!=8 & sh30!=8 & sh31!=8 & sh32!=8) %>%
          filter(!is.na(wnsee) & !is.na(wnhear) & wsh29!=8 & wsh30!=8 & wsh31!=8 & wsh32!=8) %>%
          filter(wv744a!=8 & wv744b!=8 & wv744c!=8 & wv744d!=8 & wv744e!=8) %>%
...
UGwa$wt <- UGwa$wv005/1000000
UGma$wt <- UGma$mv005/1000000

wdesg <- svydesign(id=~wv021, strata=~wv023, weights=~wt, nest = TRUE, data = UGwa) 
mdesg <- svydesign(id=~mv021, strata=~mv023, weights=~wt, nest = TRUE, data = UGma) 


save(UGwa,UGma,file="UGa.Rdata")
...
for (out in wout)
{
  tab1 <- group_by(UGwa,get(out)) %>% summarise(n=n())
...
for (out in mout)
{
  tab1 <- group_by(UGma,get(out)) %>% summarise(n=n())
  tab1$percent <- tab1$n/sum(tab1$n)*100
  tab1$print <- paste0(tab1$n," (",round(tab1$percent,2),"%)")
  print(out)
  print(tab1)
...
#Bivariate
#Women
tab1 <- data.frame(svytable(~wjust_any+wdis_any,design=wdesg,addNA=T))
tab1 <- group_by(tab1,wjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)

tab1 <- data.frame(svytable(~wjust_any+wdis_any2,design=wdesg,addNA=T))
tab1 <- group_by(tab1,wjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)
```
```{r}
#men - wdis
tab1 <- data.frame(svytable(~mjust_any+wdis_any,design=mdesg,addNA=T))
tab1 <- group_by(tab1,mjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)

tab1 <- data.frame(svytable(~mjust_any+wdis_any2,design=mdesg,addNA=T))
tab1 <- group_by(tab1,mjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)

#men-mdis
tab1 <- data.frame(svytable(~mjust_any+mdis_any,design=mdesg,addNA=T))
tab1 <- group_by(tab1,mjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)

tab1 <- data.frame(svytable(~mjust_any+mdis_any2,design=mdesg,addNA=T))
tab1 <- group_by(tab1,mjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)
```
...
tab1 <- data.frame(svytable(~mjust_any+wdis_any+mdis_any,design=mdesg,addNA=T))
tab1 <- group_by(tab1,mdis_any,mjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)

tab1 <- data.frame(svytable(~mjust_any+wdis_any2+mdis_any2,design=mdesg,addNA=T))
tab1 <- group_by(tab1,mdis_any2,mjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)
```
```{r}
# three variate (stratified by women's disability)
tab1 <- data.frame(svytable(~mjust_any+mdis_any+wdis_any,design=mdesg,addNA=T))
tab1 <- group_by(tab1,wdis_any,mjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)

tab1 <- data.frame(svytable(~mjust_any+mdis_any2+wdis_any2,design=mdesg,addNA=T))
tab1 <- group_by(tab1,wdis_any2,mjust_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)
```
```{r}
#Bivariate (X8)
#Women
tab1 <- data.frame(svytable(~wdis_any+wjust_any,design=wdesg,addNA=T))
tab1 <- group_by(tab1,wdis_any) %>% mutate(percent=Freq/sum(Freq)*100)
tab1$print <- paste0(round(tab1$Freq)," (",round(tab1$percent,2),"%)")
print(tab1)
...
for (out in c("wdis_any2","wjust_any"))
{
  for (cov in wcov)
  {
    if(cov=="wlivchild" | cov=="wwork") #svymeans does not work for missing values
    {
...
{
  for (cov in c(mcov,wcov))
  {
    #svymeans does not work for missing values
    if(cov=="mlivchild" | cov=="mwork" | cov=="wlivchild" | cov=="wwork") 
    {
      #get count and percent
...
{ 
  ...
  write.table(printt[2:4,4:5],file="test.csv",append=T,sep=",",row.names=F,col.names=T)
}
# loop for regression   
for(dis in c("wdis_any2+mdis_any2+wdis_any2*mdis_any2"))
{
  for (out in c("mjust_any"))
  {
    print("")
    print(paste0("zero order ",out,"~",dis))
    ...
    {
      print("")
      print(paste0("one adjustment ",out,"~",dis,"+",cov))
   ...
   #  print("")
   #  print(paste0("Full model ",vio," ",dis))
   #  # Dichotomized variables in full model
   # # modelobject <- svyglm(as.formula(paste0(vio,"~", dis,"+ hv105 + educ + v218 + work + wealthc + urbrur + maritalc")), design = dsgn, family = quasibinomial(link = "logit"))
   #  # None-dichotomized variables in full model
   #  modelobject <- svyglm(as.formula(paste0(vio,"~", dis,"+ age + edu + livchild + work + wealth + urbrur + maritalc")), design = dsgn, family = quasibinomial(link = "logit"))
   #  print(summary(modelobject))
   #  printer(modelobject)
  }
}
...
modelobject <- svyglm(wjust_any ~ wdis_any2 + wage+wedu+wlivchild+wwork+wwealth+wurbrur+wmarital+
                        wliteracy+wreligion+wtvradio+wfreqmedia, 
...
modelobject <- svyglm(mjust_any ~ wdis_any2 + mage+medu+mlivchild+mwork+mwealth+murbrur+mmarital+
                        mliteracy+mreligion+mtvradio+mfreqmedia+mpolygamy, 
               design = mdesg, family = quasibinomial(link = "logit"))
modelobject <- svyglm(mjust_any ~  mdis_any2 + mage+medu+mlivchild+mwork+mwealth+murbrur+mmarital+
                        mliteracy+mreligion+mtvradio+mfreqmedia+mpolygamy, 
               design = mdesg, family = quasibinomial(link = "logit"))
modelobject <- svyglm(mjust_any ~  wdis_any2 + mdis_any2 + mage+medu+mlivchild+mwork+mwealth+murbrur+mmarital+
                        mliteracy+mreligion+mtvradio+mfreqmedia+mpolygamy, 
               design = mdesg, family = quasibinomial(link = "logit"))
modelobject <- svyglm(mjust_any ~  wdis_any2 + mdis_any2 + mage+medu+mlivchild+mwork+mwealth+murbrur+mmarital+
                        mliteracy+mreligion+mtvradio+mfreqmedia+mpolygamy + wdis_any2*mdis_any2, 
               design = mdesg, family = quasibinomial(link = "logit"))
AIC(modelobject)
summary(modelobject)
```
...
{
  print(with(UGw,table(get(paste0("wv744",char)))))
}

for (num in c(1:4))
{
  print(with(UGm,table(get(paste0("mv034_",num))==whvidx)))
}

UGw$wjust <- UGw$wv744a==1|UGw$wv744b==1|UGw$wv744c==1|UGw$wv744d==1|UGw$wv744e==1

UGm2 <- UGm %>% filter(wv744a!=8&wv744b!=8&wv744c!=8&wv744d!=8&wv744e!=8&mv744a!=8&mv744b!=8&mv744c!=8&mv744d!=8&mv744e!=8)
ug3<- UGm  %>% select(sh23, sh24, sh25, sh26, sh27,sh28, sh29, sh30, sh31, sh32)
md.pattern(ug3, rotate.names = TRUE)
...
mwifeid <- unique(filter(UGm,mwifes=="Yes") %>% select(mcaseid))$mcaseid
# get if any of the wifes disabled, store in mwifevar
wda <- c()
wda2 <- c()
for (i in c(1:length(mwifeid)))
{
  test <- filter(UGm,mcaseid==mwifeid[i])
...
for (i in c(1:length(mwifeid)))
{
  UGm$wdis_any[UGm$mcaseid==mwifeid[i]] <- mwifevar[[i,2]]
  UGm$wdis_any2[UGm$mcaseid==mwifeid[i]] <- mwifevar[[i,3]]
}
# delete duplicates
UGm <- filter(UGm,mcaseid!=mlagid1)
```

# Try out child marriage
```{r}
UGwa2 <- UGwa %>% filter(whv105<=24 & whv105>=20) %>% mutate(
  marryaged=case_when(wv511<16 ~ "<16",
                      wv511<18 ~ "16-17",
                  wv511>=18 ~ ">=18")
)

wdesg <- svydesign(id=~wv021, strata=~wv023, weights=~wt, nest = TRUE, data = UGwa2)

chisq.test(UGwa2$marryaged,UGwa2$wjust_any)
svychisq(~wjust_any+marryaged,wdesg)
